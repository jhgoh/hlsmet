...
#pragma HLS UNROLL
	...
	phi_t eta13 = int(1.3 * (1<<PHI_SIZE)/6);
	phi_t eta17 = int(1.7 * (1<<PHI_SIZE)/6);
	phi_t eta25 = int(2.5 * (1<<PHI_SIZE)/6);
	phi_t eta30 = int(3.0 * (1<<PHI_SIZE)/6);

    SUM: for(int i=0; i<NPART;i++){
#pragma HLS UNROLL
		// Minus eta range is not done.

		if ( data_J_eta_int[i] < eta13 && data_J_eta_int[i] > 0 ){
			pxy_t var1 = int(0.106 * (1<<PT_SIZE) * (1<<PT_DEC_BITS)); // float to fixed type
			pxy_t var2 = int(6.6   * (1<<PT_SIZE) * (1<<PT_DEC_BITS));   // map PT_SIZE(14), 2^2 precision as J_pt ?
        	sumJ_x += (var1*jet_x[i]+var2);
        	sumJ_y += (var1*jet_y[i]+var2);
			//std::cout << " |eta| <1.3" << "\n";
		}
		if ( (data_J_eta_int[i] < eta17 && 
				data_J_eta_int[i] > eta13 ) // || 
				/*(data_J_eta_int[i] < int(-1.3 * (1<<PHI_SIZE)/6)) &&*/
				/*data_J_eta_int[i] > int(-1.7 * (1<<PHI_SIZE)/6) */ ){
			pxy_t var1 = int(0.216 * (1<<PT_SIZE) * (1<<PT_DEC_BITS));
			pxy_t var2 = int(5.5   * (1<<PT_SIZE) * (1<<PT_DEC_BITS));
        	sumJ_x += (var1*jet_x[i]+var2);
        	sumJ_y += (var1*jet_y[i]+var2);
			//std::cout << " 1.3 |eta| <1.7" << "\n";
		}
		if ( data_J_eta_int[i] < eta30 && 
				data_J_eta_int[i] > eta25 ){
			pxy_t var1 = int(0.083 * (1<<PT_SIZE) * (1<<PT_DEC_BITS));
			pxy_t var2 = int(13.2  * (1<<PT_SIZE) * (1<<PT_DEC_BITS));
        	sumJ_x += (var1*jet_x[i]+var2);
        	sumJ_y += (var1*jet_y[i]+var2);
			//std::cout << "2.5< |eta| <3.0" << "\n";
		}
        if(DEBUG){
	...
	corr_x = met_x + (K_value * (sumJ_x >> PT_SIZE));
	corr_y = met_y + (K_value * (sumJ_y >> PT_SIZE));

	corr_pt2 = corr_x*corr_x + corr_y*corr_y; 
    //PhiFromXY(sum_x,sum_y,res_phi);

	std::cout << " corr_pt2 (HW unit) = " << corr_pt2 << " \n";
